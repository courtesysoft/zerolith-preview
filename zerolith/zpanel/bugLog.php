...
extract(zfilter::array("zpta|zptafilter|action|action|debugID|zOrd|debugReason|time|visitURL|visitIP|userID|userName", "URL"));

if($action == "showDebug")
{
	zl::setOutFormat("html");
	$debugData = zdb::getRow("SELECT * FROM zl_debug WHERE ID='" . intval($debugID) . "'", "could not load email ID: " . $debugID);
	?>
		...
		<div class="zl_right zl_w75p zl_debugStored">
			<?=gzuncompress($debugData['debugDump'])?>
		</div>
	<?php
...
//initialize
if($zpta != "Y") { zpage::start("ZL Debug Log"); }
...
if($zpta != "Y")
{
	...
	<tr>
		...
		<td class="zl_padT3">Time:</td><td class="zl_padT3"><?=zui::textBox("time", $time, "zl_w150", 'hx-get="?zpta=Y" hx-include="#searchBox, #zOrd" hx-trigger="keyup changed delay:250ms, search"', "search")?></td>
		<td class="zl_padT3">URL:</td><td class="zl_padT3"><?=zui::textBox("visitURL", $visitURL, "zl_w150", 'hx-get="?zpta=Y" hx-include="#searchBox, #zOrd" hx-trigger="keyup changed delay:250ms, search"', "search")?></td>
	</tr>
    <tr>
	    <td>IP:</td><td><?=zui::textBox("visitIP",$visitIP, "zl_w150", 'hx-get="?zpta=Y" hx-include="#searchBox, #zOrd" hx-trigger="keyup changed delay:250ms, search"', "search")?></td>
	    <td>UserID:</td><td><?=zui::textBox("userID", $userID, "zl_w150", 'hx-get="?zpta=Y" hx-include="#searchBox, #zOrd" hx-trigger="keyup changed delay:250ms, search"', "search")?></td>
	    <td>UserName:</td><td><?=zui::textBox("userName", $userName, "zl_w150", 'hx-get="?zpta=Y" hx-include="#searchBox, #zOrd" hx-trigger="keyup changed delay:250ms, search"', "search")?></td>
    </tr>
    </table>
	</form>
	<br />
...
foreach($PTAarray as $row)
{
	...
	//show a debug frame.
	$row['time'] = '<a href="" hx-target="#debugPreview" hx-get="?action=showDebug&debugID=' . $row['ID'] . '" onclick="' . "document.querySelectorAll('.zPTA tr').forEach((element) => { element.classList.remove('zl_bgBlueRow'); }); this.parentElement.parentElement.classList.add('zl_bgBlueRow');" . '">' . ztime::formatTimestamp($row['time']) . '</a></span>';
